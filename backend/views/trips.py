...
def add_trips():
    ...
    # Automatically update status if the end date has passed and status is still Pending
    status = TripStatusEnum.PENDING.value  # Default to 'Pending'
    if end_date < today:
        status = TripStatusEnum.COMPLETED.value  # Automatically set to 'Completed' if the trip has ended
...
def update_trips(trip_id):
    ...
    if trip and trip.user_id == current_user_id:
        ...
        # If start_date or end_date are provided, validate the date format
        if start_date:
            try:
                start_date = datetime.strptime(start_date, '%Y-%m-%d').date()
            except ValueError:
        ...
        # Automatically update the status if the end date has passed
        if end_date and end_date < today and trip.status != TripStatusEnum.COMPLETED.value:
            trip.status = TripStatusEnum.COMPLETED.value
